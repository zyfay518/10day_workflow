...
-- ==========================================
-- TRIGGER: Auto-create user profile and default dimensions
-- ==========================================
...
BEGIN
  ...
  VALUES
    ...
    (new.id, 'Study', '#a3b8a6', 'Book', 2),
    (new.id, 'Wealth', '#e8d5c4', 'DollarSign', 3),
    (new.id, 'Family', '#c49eb3', 'Users', 4),
    (new.id, 'Leisure', '#9ca3af', 'Coffee', 5);
...
-- ==========================================
-- 18. 语音快记入口表 (voice_quick_entries)
-- ==========================================
CREATE TABLE IF NOT EXISTS public.voice_quick_entries (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  ...
  parse_mode TEXT NOT NULL CHECK (parse_mode IN ('parsed_to_records_goals', 'saved_to_library')),
...
CREATE INDEX IF NOT EXISTS idx_voice_quick_entries_user_created
  ON public.voice_quick_entries(user_id, created_at DESC);

-- ==========================================
-- 19. 语音解析草稿表 (voice_parsed_drafts)
-- ==========================================
CREATE TABLE IF NOT EXISTS public.voice_parsed_drafts (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  voice_entry_id BIGINT NOT NULL REFERENCES public.voice_quick_entries(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE(voice_entry_id)
);

CREATE INDEX IF NOT EXISTS idx_voice_parsed_drafts_user_created
  ON public.voice_parsed_drafts(user_id, created_at DESC);
